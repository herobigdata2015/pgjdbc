...
import org.herodbsql.core.v3.BatchedQuery;
import org.herodbsql.largeobject.LargeObject;
...
import org.herodbsql.util.PGTime;
import org.herodbsql.util.PGTimestamp;
import org.herodbsql.util.PGobject;
import org.herodbsql.util.PSQLException;
import org.herodbsql.util.PSQLState;
import org.herodbsql.util.ReaderInputStream;
...
//    setDate(parameterIndex, x, null);
    ...
    setTimestamp(parameterIndex, ts, null);
  }
...
//      }
//    }
    byte[] data = myCreateBytea(x.getBinaryStream(), -1);
    setBytes(i, data);
  }
...
//      setNull(i, Types.CLOB);
      setString(i, null, getStringType());
      return;
    }

    long len = x.length();
    String str = x.getSubString(1, (int) len);
...
//    throw Driver.notImplemented(this.getClass(), "setClob(int, Reader, long)");
    setString(parameterIndex, new String(myCreateClob(reader, length)));
  }

  private char[] myCreateClob(Reader reader, long length) throws SQLException {
    ...
    try {
      ...
      int numRead = reader.read(cbuf, 0,
          (length > 0 && remaining < cbuf.length ? (int) remaining : cbuf.length));
      while (numRead != -1 && remaining > 0) {
        remaining -= numRead;
        writer.write(cbuf, 0, numRead);
        numRead = reader.read(cbuf, 0,
            (length > 0 && remaining < cbuf.length ? (int) remaining : cbuf.length));
      }
      writer.flush();
    } catch (IOException se) {
      throw new PSQLException(GT.tr("Unexpected error writing myclob."),
          PSQLState.UNEXPECTED_ERROR, se);
    } finally {
      try {
        writer.close();
...
//    throw Driver.notImplemented(this.getClass(), "setClob(int, Reader)");
    setString(parameterIndex, new String(myCreateClob(reader, -1)));
  }
...
//    setLong(parameterIndex, oid);
    setBytes(parameterIndex, myCreateBytea(inputStream, length));
  }

  private byte[] myCreateBytea(InputStream inputStream, long length) throws SQLException {
    ...
    try {
      ...
      throw new PSQLException(GT.tr("Unexpected error writing bytea."),
          PSQLState.UNEXPECTED_ERROR, se);
    } finally {
      try {
        outputStream.close();
...
//    setLong(parameterIndex, oid);
    setBytes(parameterIndex, myCreateBytea(inputStream, -1));
  }
