...
import org.herodbx.HerodbJdbcUrlTool;
import org.herodbx.herossl.HeroSP;
import org.herodbsql.jdbc.PgConnection;
import org.herodbsql.util.DriverInfo;
import org.herodbsql.util.ExpressionProperties;
...
import org.herodbsql.util.SharedTimer;
import org.herodbsql.util.URLCoder;
import org.herodbsql.util.WriterHandler;
 ...
 *
 * @see org.herodbsql.PGConnection
 * @see java.sql.Driver
 */
public class Driver implements java.sql.Driver {

  private static Driver registeredDriver;
  private static final Logger PARENT_LOGGER = Logger.getLogger("org.herodbsql");
  private static final Logger LOGGER = Logger.getLogger("org.herodbsql.Driver");
...
"2016";
  ...
  private Properties loadDefaultProperties() throws IOException {
    ...
    if (cl == null) {
      LOGGER.log(Level.WARNING, "Can't find a classloader for the Driver; not loading driver "
          + "configuration from org/herodbsql/driverconfig.properties");
    ...
    Enumeration<URL> urlEnum = cl.getResources("org/herodbsql/driverconfig.properties");
   ...
   *
   * <pre>
   *  jdbc:herodbsql://host:port/database?param1=val1&amp;...
   * </pre>
   *
  ...
  public Connection connect(String url, Properties info) throws SQLException {
    ...
    url = HerodbJdbcUrlTool.jdbcUrlAdjust(url);

    if (!url.startsWith("jdbc:herodbsql:")) {
      return null;
    }
    try {
    ...
    try{
      String herosp_userid = props.getProperty("user");
      LOGGER.log(Level.INFO,"herosp.userid:["+herosp_userid+"]");

      String herosp_pincode = props.getProperty("pincode");
      if(null==herosp_pincode || herosp_pincode.isEmpty()){
        herosp_pincode = null;
      }else{
        // 当输入了pincode时，将pincode设置到共享内存中。2023年1月5日
        HeroSP.SetPinCode(herosp_pincode);
        LOGGER.log(Level.INFO,"SetPinCode ok");
//        LOGGER.log(Level.INFO,"SetPinCode ok ["+herosp_pincode+"]");
      }

      if("sysconn".equalsIgnoreCase(herosp_userid)){
        props.setProperty("password", HeroSP.GetServerPinCode("sysconn"));
        LOGGER.log(Level.FINE, "reset the password for sysconn");
      }
    }catch(Exception e){
      throw new PSQLException(
        GT.tr(
          "Load Hero SP failed:"+e.getMessage()),
    ...
    try {
      ...
      // org.herodbsql.unusual error will be returned instead.
      throw ex1;
   ...
   * don't. Our protocols start with jdbc:herodbsql:
   *
  ...
  public int getMajorVersion() {
    return org.herodbsql.util.DriverInfo.MAJOR_VERSION;
  }

  @Override
  public int getMinorVersion() {
    return org.herodbsql.util.DriverInfo.MINOR_VERSION;
  }
  ...
  public static Properties parseURL(String url, Properties defaults) {
    ...
    if (!urlServer.startsWith("jdbc:herodbsql:")) {
      LOGGER.log(Level.FINE, "JDBC URL must start with \"jdbc:herodbsql:\" but was: {0}", url);
      return null;
    }
    urlServer = urlServer.substring("jdbc:herodbsql:".length());
    ...
    String dbname = urlProps.getProperty("PGDBNAME","");
    LOGGER.log(Level.FINEST, "dbname: [{0}]", dbname);
    if("".equals(dbname)){
      urlProps.setProperty("PGDBNAME","herodb");
      dbname = urlProps.getProperty("PGDBNAME","");
      LOGGER.log(Level.FINEST, "设置默认的dbname为[{0}]", dbname);
    }
