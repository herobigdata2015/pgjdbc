...
import org.herodbsql.util.NullOutputStream;
...
/*
 * Tests the dynamically created class org.herodbsql.Driver
 *
 */
public class DriverTest {
  ...
  public void testAcceptsURL() throws Exception {
    ...
    org.herodbsql.Driver drv = new org.herodbsql.Driver();
    assertNotNull(drv);

    // These are always correct
    verifyUrl(drv, "jdbc:herodbsql:test", "localhost", "5432", "test");
    verifyUrl(drv, "jdbc:herodbsql://localhost/test", "localhost", "5432", "test");
    verifyUrl(drv, "jdbc:herodbsql://localhost:5432/test", "localhost", "5432", "test");
    verifyUrl(drv, "jdbc:herodbsql://127.0.0.1/anydbname", "127.0.0.1", "5432", "anydbname");
    verifyUrl(drv, "jdbc:herodbsql://127.0.0.1:5433/hidden", "127.0.0.1", "5433", "hidden");
    verifyUrl(drv, "jdbc:herodbsql://[::1]:5740/db", "[::1]", "5740", "db");

    // Badly formatted url's
    assertFalse(drv.acceptsURL("jdbc:postgres:test"));
    assertFalse(drv.acceptsURL("postgresql:test"));
    assertFalse(drv.acceptsURL("db"));
    assertFalse(drv.acceptsURL("jdbc:herodbsql://localhost:5432a/test"));
    assertFalse(drv.acceptsURL("jdbc:herodbsql://localhost:500000/test"));
    assertFalse(drv.acceptsURL("jdbc:herodbsql://localhost:0/test"));
    assertFalse(drv.acceptsURL("jdbc:herodbsql://localhost:-2/test"));

    // failover urls
    verifyUrl(drv, "jdbc:herodbsql://localhost,127.0.0.1:5432/test", "localhost,127.0.0.1",
        "5432,5432", "test");
    verifyUrl(drv, "jdbc:herodbsql://localhost:5433,127.0.0.1:5432/test", "localhost,127.0.0.1",
        "5433,5432", "test");
    verifyUrl(drv, "jdbc:herodbsql://[::1],[::1]:5432/db", "[::1],[::1]", "5432,5432", "db");
    verifyUrl(drv, "jdbc:herodbsql://[::1]:5740,127.0.0.1:5432/db", "[::1],127.0.0.1", "5740,5432",
        "db");
  }
  ...
  public void testConnectFailover() throws Exception {
    String url = "jdbc:herodbsql://invalidhost.not.here," + TestUtil.getServer() + ":"
  ...
  public void testSetLogWriter() throws Exception {
    ...
    try {
      ...
      Logger logger = Logger.getLogger("org.herodbsql");
